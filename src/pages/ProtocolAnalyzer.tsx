...
import { protocolComparison, aiLogs, protocolPerformanceData } from '../data/mockData';
    ...
    return (
        <div className="p-8 lg:p-12 space-y-8 max-w-7xl mx-auto w-full">
            ...
            <section className="relative group">
                <div className="absolute -inset-0.5 bg-gradient-to-r from-primary to-primary-container rounded-2xl blur opacity-30 group-hover:opacity-50 transition duration-1000 animate-pulse"></div>
                <div className="relative bg-surface-container-high p-8 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-8 border border-primary/20">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-display text-sm uppercase tracking-widest text-primary-fixed">OPTIMAL SIGNAL FOUND</span>
                        </div>
                        <h2 className="font-display text-3xl md:text-5xl font-black text-on-surface tracking-tight">
                            Best Protocol: <span className="text-primary">HTTP/3</span>
                        </h2>
                        <p className="text-on-surface-variant text-lg">Reason: Lower latency under packet loss due to QUIC multiplexing.</p>
                    </div>
                    
                    <div className="flex items-center gap-8">
                        <div className="text-center">
                            <div className="text-5xl font-display font-bold text-primary">87%</div>
                            <div className="text-[10px] font-label uppercase tracking-widest text-on-surface-variant mt-1">CONFIDENCE</div>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-surface-container-low rounded-2xl p-6 flex flex-col gap-6 border border-outline-variant/5 shadow-lg">
                    <div className="flex justify-between items-end flex-wrap gap-4">
                        <div>
                            ...
                            <p className="text-xs text-on-surface-variant uppercase tracking-widest">REAL-TIME TELEMETRY (ms)</p>
                        </div>
                    ...
                    <div className="h-64 w-full relative mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={protocolPerformanceData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                                ...
                                />
                                <Area type="monotone" dataKey="http3" stroke="#55d8e1" strokeWidth={3} fill="#55d8e1" fillOpacity={0.1} />
                                <Area type="monotone" dataKey="http2" stroke="#ffb68d" strokeWidth={2} fill="transparent" />
                                <Area type="monotone" dataKey="udp" stroke="#ffb4ab" strokeWidth={2} fill="transparent" strokeDasharray="4 4" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-surface-container-low rounded-2xl p-6 space-y-8 border border-outline-variant/5 shadow-lg">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between p-4 bg-surface-container-highest rounded-xl border border-primary/30 shadow-[0_0_10px_rgba(0,173,181,0.1)] cursor-pointer">
                            <span className="text-sm font-medium text-primary">Packet Loss (5%)</span>
                        ...
                        <div className="flex items-center justify-between p-4 bg-surface-container-highest rounded-xl border border-outline-variant/10 hover:border-outline-variant/30 transition-colors cursor-pointer">
                            <span className="text-sm font-medium">Stable Network</span>
                            <div className="w-10 h-5 bg-surface rounded-full relative p-1 transition-colors">
                                <div className="w-3 h-3 bg-slate-400 rounded-full"></div>
                            </div>
                        </div>
                    ...
                    >
                        {isTesting ? <div className="w-5 h-5 border-2 border-on-primary-container border-t-transparent rounded-full animate-spin"></div> : 'Run Benchmark Test'}
                    </button>
                </div>
                ...
                <div className="lg:col-span-3 bg-surface-container-low rounded-2xl overflow-hidden shadow-lg border border-outline-variant/5">
                    <div className="p-6 border-b border-outline-variant/10">
                        ...
                        <p className="text-xs text-on-surface-variant uppercase tracking-widest">REAL-TIME COMPARISON</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-surface-container-highest/50">
                                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Protocol</th>
                                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Latency</th>
                                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Packet Loss</th>
                                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Throughput</th>
                                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Stability</th>
                                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] font-bold text-slate-500">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-outline-variant/10">
                                {protocolComparison.map((p, i) => (
                                    <tr key={i} className={p.active ? "bg-primary/5 border-l-4 border-primary" : "hover:bg-surface-container-highest/30 transition-colors"}>
                                        <td className={`px-6 py-6 font-display font-bold ${p.active ? 'text-primary' : 'text-on-surface'}`}>{p.name}</td>
                                        <td className="px-6 py-6 text-sm">{p.latency}</td>
                                        <td className="px-6 py-6 text-sm">{p.packetLoss}</td>
                                        <td className="px-6 py-6 text-sm">{p.throughput}</td>
                                        ...
                                        <td className="px-6 py-6">
                                            <span className={`px-3 py-1 text-[10px] font-bold uppercase rounded-full ${
                                                p.status === 'WINNER' ? 'bg-primary/20 text-primary border border-primary/20' : 
                                                p.status === 'ACTIVE' ? 'bg-surface-variant text-slate-300' : 
                ...
                <div className="lg:col-span-3 bg-[#080f17] rounded-2xl p-6 border border-outline-variant/20 font-mono text-xs overflow-hidden group shadow-inner">
                    <div className="flex items-center justify-between mb-4 pb-4 border-b border-outline-variant/10">
                        ...
                        <div className="text-slate-500 uppercase tracking-widest text-[10px] font-bold">AI REASONING LOGS // KINETIC_SIGNAL_v4.2</div>
                    </div>
                        ...
                        {aiLogs.map((log, i) => (
                            <p key={i} className={log.animate ? "animate-pulse" : ""}>
                                ...
                                <span className={`font-bold mr-2 ${
                                    ...
                                    log.type === 'ANALYZING' ? 'text-tertiary' : 
                                    log.type === 'DECISION' ? 'text-error' : 'text-primary-fixed'
                                }`}>{log.type}</span> 
